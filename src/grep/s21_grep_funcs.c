...
static char *add_to_string(char *string, const char *word) {
  ...
  if (string) { /*if string of patterns wasn't empty*/
    ...
    new_string = (char *)realloc(string, sizeof(char) * new_length);
    if (new_string) {
...
static char *load_patterns(char *patterns, const char *filename,
                           const dflag flag) {
  FILE *patfile;
  ...
  while ((res = getopt_long(argc, argv, short_options, 0, 0)) != -1) {
    switch (res) {
      case 'e': {
        ...
        *(argv + optind) =
            NULL; /*except pattern from arguments of command line*/
        optind++;
        break;
      }
      case 'i': {
...
static void output(regex_t *regex, int *result, char *line_buf,
                   const char *filename, const int multifile, const dflag flag,
                   const int line, regmatch_t match) {
  if (flag.v) {
    *result = !(*result);
  }
  if (!(*result)) {
    ...
    if (flag.o) {
      printf("%.*s\n", (int)(match.rm_eo - match.rm_so),
             line_buf + match.rm_so);
      char *remain_line =
          line_buf + match.rm_eo; /*stand in end of first match*/
      *result =
          regexec(regex, remain_line, 1, &match, 0); /*exec with remain string*/
      while (!(*result)) { /*while there are matches print they*/
        output(regex, result, remain_line, filename, multifile, flag, line,
               match);
      }
    } else {
      printf("%s", line_buf);
...
static void output_c(int line_counter, int line, dflag flag, char *filename,
                     int multifile) {
  if (flag.v) {
...
void files_controller(int optind, const int argc, char **argv, const dflag flag,
  ...
  if (!(*patterns)) { /*if there wasn't -e option, then first argument is
                         pattern*/
    *patterns = add_to_string(*patterns, argv[optind++]);
  }
